...
function areUrlsEquivalent(url1, url2) {
  var _url1 = url.parse(url1);
  var _url2 = url.parse(url2);

  if (!_url1 || !_url2) return false;

  return (_url1.href == _url2.href);
}
...
if (options.errorIfStatusIsNot200) {
  page.onResourceReceived = function(response) {
    ...
    if (areUrlsEquivalent(response.url, site) && response.status !== 200) {
